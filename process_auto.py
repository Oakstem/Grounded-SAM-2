...
from grounding_dino.groundingdino.util.inference import predict, load_model
        ...
        if person_points_file:
            try:
                ...
                results["person"] = {
                    ...
                    "boxes": person_results["boxes"].tolist() if isinstance(person_results["boxes"], np.ndarray) else
        ...
        for gaze_file in gaze_points_files:
            ...
            try:
                ...
                results["gaze_targets"][gaze_id] = {
                    ...
                    "boxes": gaze_results["boxes"].tolist() if isinstance(gaze_results["boxes"], np.ndarray) else
                    gaze_results["boxes"],
                    # We don't store the masks to save space
                }
            except Exception as e:
